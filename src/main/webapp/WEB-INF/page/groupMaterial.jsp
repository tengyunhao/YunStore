		...
		<div class="layui-row" style="margin-top: 10px;background-color:#fcfcfc">
			<div class="layui-collapse">
				...
				<div class="layui-colla-item">
					...
					<button id="add-group" class="layui-btn">创建资料组</button>
					<button id="apply-group" class="layui-btn">加入资料组</button>
...
<script type="text/html" id="user">
	<i class="layui-icon" style="float:left; font-size: 20px; color: #9fbaed;">&#xe612;</i>
	<div style="float:left"><lable>&nbsp&nbsp{{ d.name }}</lable></div>
</script>
<script>
var nowGroup = "";
...
layui.use(['table','element'], function() {
	...
	table.init('groupEvent', {
		id: 'group',
		url: '${pageContext.request.contextPath }/list-group-material.do',
		height: 660,
		limit: 10, //注意：请务必确保 limit 参数（默认：10）是与你服务端限定的数据条数一致
		size: 'lg',
		cols: [[
			{field:'name', title:'我的群组', event: 'enter', templet: '#group'}
		]]
	});
	...
	$("#add-group").click(function() {
		layer.prompt({title: '要创建资料组的名称', formType: 0}, function(name, index){
			layer.load();
			$.ajax({
				type : "post",
				url : "${pageContext.request.contextPath }/add-group-material.do",
				data : {
					name : name
				},
	...
	table.on('tool(groupEvent)', function(obj) { //注：tool是工具条事件名，demoEvent是table原始容器的属性 lay-filter="对应的值"
		...
		if (event === 'enter') {
			...
			table.render({
				...
				skin: 'line',
				size: 'lg',
				url: '${pageContext.request.contextPath }/file-group-material.do', //数据接口
				where: {
					group : data.id
				},
				cols: [[
			...
			table.render({
				...
				skin: 'line',
				size: 'lg',
				url: '${pageContext.request.contextPath }/user-group-material.do', //数据接口
				where: {
					group : data.id
				},
				cols: [[
					{field:'id', checkbox: true},
					{field:'name', title: '成员', width:150, event: 'enter', style:'cursor: pointer;', sort: true, templet: '#user'},
	...
	$("#file-share").click(function(){
		layer.open({
			...
			yes: function(index, layero){
				...
				$.ajax({
					type : "post",
					url : "${pageContext.request.contextPath }/share-group-material.do",
					data : { file : nowFile, group : nowGroup },
					success : function(data) {
						table.reload('files', {
							url: '${pageContext.request.contextPath }/file-group-material.do',
